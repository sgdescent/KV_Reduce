...
We evaluate the six target--draft pairs in Table~\ref{tab:model-pairs} using
contiguous WikiText-2 validation blocks, four draft proposals per verification
round, greedy target verification, and 16 generated tokens per prompt. Main 1K
and 4K experiments use 32 and 12 prompts per pair. We add two shuffled C4 seeds
for Qwen2.5-3B/1.5B, OLMo-2, and Llama, and exploratory Qwen2.5 runs at 8K and
16K. Sensitivity profiles use eight prompts per candidate; held-out allocation
evaluation uses 32. All pairs were checked for tokenizer compatibility.

...
for o_budget in $O_BUDGETS; do
  ...
  fit_job=$(
    sbatch --parsable \
      ...
      --job-name="$fit_name" \
      --export=ALL,ENABLE_WANDB="$ENABLE_WANDB",WANDB_PROJECT="$WANDB_PROJECT",WANDB_GROUP="$WANDB_GROUP",WANDB_ENTITY="$WANDB_ENTITY",WANDB_RUN_NAME="$fit_name",WANDB_LOG_INTERVAL="$WANDB_LOG_INTERVAL",BIG_MODEL="$BIG_MODEL",SMALL_MODEL="$SMALL_MODEL",DATASET_NAME="$DATASET_NAME",DATASET_CONFIG="$DATASET_CONFIG",SEQ_LEN="$SEQ_LEN",TRAIN_SEQUENCES="$o_budget",K_TRAIN_SEQUENCES="$K_TRAIN_SEQUENCES",O_TRAIN_SEQUENCES="$o_budget",LAMBDA_REG="$LAMBDA_REG",OUTPUT_ROUTING_SOURCE="$OUTPUT_ROUTING_SOURCE",LAYER_MAP_FILE="$LAYER_MAP_FILE",OUT_DIR="$fit_out" \

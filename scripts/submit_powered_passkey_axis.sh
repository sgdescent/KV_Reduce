...
eval_job=$(sbatch --parsable --array=0-2%1 \
  "${dependency_args[@]}" \
  --export="ALL,MODEL=Qwen/Qwen2.5-1.5B,MODEL_TAG=qwen25_15b_per_token_powered,OUT_ROOT=$per_token_root,PASSKEY_CONTEXTS=16384,PASSKEY_EXAMPLES=64,PASSKEY_NUM_CHOICES=16,PASSKEY_VARIANT=confusable_records,PASSKEY_SCORE=raw,QUANT_CONFIGS=none;k4v4;k4v2;k2v4;k2v2,KEY_QUANT_AXIS=per_token,KEY_GROUP_SIZE=32,KEY_RESIDUAL_LENGTH=0,VALUE_QUANT_SCHEME=affine,WANDB_PROJECT=kv-reduce,PASSKEY_WANDB_GROUP=passkey-axis-powered-per-token" \
...
axis_aggregate=$(sbatch --parsable \
  ...
  --export="ALL,PER_CHANNEL_ROOT=$per_channel_root,PER_TOKEN_ROOT=$per_token_root,OUT_DIR=$comparison_root,PASSKEY_CONTEXT=16384,PASSKEY_EXAMPLES=64,PASSKEY_NUM_CHOICES=16,PASSKEY_GENERATOR_VERSION=synthetic_associative_passkey_v3,PASSKEY_VARIANT=confusable_records,PASSKEY_SCORE=raw" \

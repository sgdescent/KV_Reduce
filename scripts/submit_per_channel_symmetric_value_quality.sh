...
root="${ROOT:-outputs/quantizer_factorial/per_channel_key_symmetric_value/qwen25_3b_15b}"
...
quality_job=$(sbatch --parsable \
  ...
  --array=0-5%1 \
  --exclude="$exclude" \
  --export="ALL,KEY_QUANT_AXIS=per_channel,KEY_GROUP_SIZE=32,KEY_RESIDUAL_LENGTH=128,VALUE_QUANT_SCHEME=symmetric,QUALITY_ROOT=${root}/quality,QUANT_CONFIGS=${configs},WANDB_PROJECT=kv-reduce,WANDB_GROUP=quantizer-factorial-per-channel-symmetric-quality" \
...
printf 'Submitted per-channel-key/symmetric-value quality grid\n  quality: %s\n  aggregate: %s\n' \

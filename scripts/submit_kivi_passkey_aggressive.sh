...
aggregate_job=$(sbatch --parsable \
  ...
  --exclude=catalyst-0-9,catalyst-0-15 \
  --export="ALL,PASSKEY_EXAMPLES=${PASSKEY_EXAMPLES:-16},PASSKEY_NUM_CHOICES=$num_choices,PASSKEY_VARIANT=$variant,PASSKEY_SCORE=$score,PASSKEY_GENERATOR_VERSION=$generator_version,OUT_ROOT=$out_root" \

...
def rollout_metric_prefix(prefix: str, ref_tokens: List[int], test_tokens: List[int]) -> Dict[str, float]:
    prefix_match = first_mismatch_position(ref_tokens, test_tokens)
    match_rate = 0.0
    if ref_tokens:
        match_rate = sum(int(a == b) for a, b in zip(ref_tokens, test_tokens)) / float(len(ref_tokens))
    return {
        f"{prefix}_rollout_exact_match": float(ref_tokens == test_tokens),
        f"{prefix}_rollout_prefix_match_len": float(prefix_match),
        f"{prefix}_rollout_token_match_rate": float(match_rate),
    }
...
@torch.no_grad()
def greedy_rollout_from_state(model, first_logits: torch.Tensor, past_key_values, steps: int) -> List[int]:
    if steps <= 0:
        return []
    next_token = first_logits.argmax(dim=-1, keepdim=True)
    if next_token.shape[0] != 1:
        raise ValueError("greedy_rollout_from_state assumes batch size 1")
...
def train_v_mlps(
    ...
    for layer_idx, (x_cpu, y_cpu) in enumerate(zip(v_x_by_layer, v_y_by_layer)):
        if x_cpu.numel() == 0:
...
def main() -> None:
    ...
    for train_idx, block in enumerate(train_iter):
        ...
        for small_layer_idx, big_layer_idx in enumerate(layer_map):
            ...
            xk = flatten_kv(k_big).float()[:: args.position_stride].to(args.big_device)
            xv = flatten_kv(v_big).float()[:: args.position_stride].to(args.big_device)
            yk = flatten_kv(k_small).float()[:: args.position_stride].to(args.small_device)
            yv = flatten_kv(v_small).float()[:: args.position_stride].to(args.small_device)
            ...
            if remaining > 0:
                ...
                if take < xv.shape[0]:
                    perm = torch.randperm(xv.shape[0], device=xv.device)[:take]
                    xv_keep = xv[perm]
                    yv_keep = yv[perm]
                else:
                    xv_keep = xv
                    yv_keep = yv
                v_x_parts[small_layer_idx].append(xv_keep.cpu().to(torch.float16))
                v_y_parts[small_layer_idx].append(yv_keep.cpu().to(torch.float16))
    ...
    for eval_idx, block in enumerate(eval_iter):
        ...
        if args.generate_steps > 0:
            big_roll = greedy_rollout_from_state(
                model=big_model,
                first_logits=big_next_out.logits[:, -1, :],
                past_key_values=big_next_out.past_key_values,
                steps=args.generate_steps,
            )
            native_roll = greedy_rollout_from_state(
                model=small_model,
                first_logits=small_native_next_out.logits[:, -1, :],
                past_key_values=small_native_next_out.past_key_values,
                steps=args.generate_steps,
            )
            ridge_roll = greedy_rollout_from_state(
                model=small_model,
                first_logits=eval_outputs["ridge"].logits[:, -1, :],
                past_key_values=eval_outputs["ridge"].past_key_values,
                steps=args.generate_steps,
            )
            hybrid_roll = greedy_rollout_from_state(
                model=small_model,
                first_logits=eval_outputs["hybrid"].logits[:, -1, :],
                past_key_values=eval_outputs["hybrid"].past_key_values,
                steps=args.generate_steps,
            )
            konly_roll = greedy_rollout_from_state(
                model=small_model,
                first_logits=eval_outputs["konly"].logits[:, -1, :],
                past_key_values=eval_outputs["konly"].past_key_values,
                steps=args.generate_steps,
            )
            mlpvonly_roll = greedy_rollout_from_state(
                model=small_model,
                first_logits=eval_outputs["mlpvonly"].logits[:, -1, :],
                past_key_values=eval_outputs["mlpvonly"].past_key_values,
                steps=args.generate_steps,
            )

            next_row.update(rollout_metric_prefix("native_vs_big", big_roll, native_roll))
            next_row.update(rollout_metric_prefix("ridge_vs_big", big_roll, ridge_roll))
            next_row.update(rollout_metric_prefix("hybrid_vs_big", big_roll, hybrid_roll))
            next_row.update(rollout_metric_prefix("konly_vs_big", big_roll, konly_roll))
            next_row.update(rollout_metric_prefix("mlpvonly_vs_big", big_roll, mlpvonly_roll))
            next_row.update(rollout_metric_prefix("hybrid_vs_native", native_roll, hybrid_roll))

            if identity_possible:
                identity_roll = greedy_rollout_from_state(
                    model=small_model,
                    first_logits=eval_outputs["identity"].logits[:, -1, :],
                    past_key_values=eval_outputs["identity"].past_key_values,
                    steps=args.generate_steps,
                )
                next_row.update(rollout_metric_prefix("identity_vs_big", big_roll, identity_roll))
